...
Strike.onclick = function() {
  ...
  for (i=0;i<Params.nOsc;i++) {
    gNode[i]=context.createGain();
    osc[i]= context.createOscillator();
    osc[i].start();
    osc[i].connect(gNode[i]);
  }
  ...
  for (i=0;i<Params.nOsc;i++) {
    ...
    gNode[i].gain.exponentialRampToValueAtTime(DBToAmp(Params.Amp0[i]+Params.Slope[i]*8/Duration.value),now+8);
    gNode[i].gain.linearRampToValueAtTime(0, now + 12);
    gNode[i].connect(context.destination);
  }
}
Params= { //BigBen:
  nOsc:14,
